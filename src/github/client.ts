...
export class GitHubClient {
  ...
  async getPRComments(prNumber: number): Promise<Array<{ id: number; body: string; user: { login: string } }>> {
    return this.withRetry(async () => {
      debug(`Fetching comments for PR #${prNumber}`);
      ...
      return response.data.map(comment => ({
        id: comment.id,
        body: comment.body || '',
        user: comment.user ? { login: comment.user.login } : { login: 'unknown' },
      }));
  ...
  async updatePRComment(commentId: number, body: string): Promise<void> {
    await this.withRetry(async () => {
      debug(`Updating comment ${commentId}`);
    ...
    }, `updatePRComment(${commentId})`);
  }

  /**
   * Post or update a comment on a pull request
   * Updates existing Kakarot CI comments, creates new one if none exists
   */
  async commentPR(prNumber: number, body: string, updateExisting = true): Promise<void> {
    if (updateExisting) {
      try {
        // Find existing Kakarot CI comments
        const comments = await this.getPRComments(prNumber);
        const kakarotComment = comments.find(comment => 
          comment.body.includes('*Generated by [Kakarot CI]') ||
          comment.body.includes('Generated by Kakarot CI')
        );

        if (kakarotComment) {
          debug(`Updating existing Kakarot CI comment ${kakarotComment.id}`);
          await this.updatePRComment(kakarotComment.id, body);
          return;
        }
      } catch (err) {
        warn(`Failed to check for existing comments, creating new one: ${err instanceof Error ? err.message : String(err)}`);
      }
    }

    // Create new comment if no existing one found or updateExisting is false
    await this.withRetry(async () => {
      debug(`Posting new comment on PR #${prNumber}`);
